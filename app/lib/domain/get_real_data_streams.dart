...
class GetRealDataStreams {
  ...
  GetRealDataStreams(this._sensorsPort,
  ...
  Stream<Position> positionStream() {
    return _sensorsPort.sensorsStreams.accelerometerStream.map((SensorData acc) {
      if (_lastTimestamp == null) {
        _lastTimestamp = acc.timestamp;
        return Position(x: _positionX, y: _positionY, timestamp: acc.timestamp);
      }

      // Compute time difference in seconds
      double dt = acc.timestamp.difference(_lastTimestamp!).inMilliseconds / 1000.0;
      _lastTimestamp = acc.timestamp;
      ...
      _velocityX -= 500 * accX * dt;
      _velocityY += 500 * accY * dt;
