...
import 'package:app/adapter/tflite_model_adapter/inner/recognition.dart';
...
class Detector {
  ...
  static Future<Detector> start(String modelPath, String labelPath) async {
  ...
  static Future<Interpreter> _loadModel(String modelPath) async {
    ...
    return Interpreter.fromAsset(modelPath, options: interpreterOptions..threads = 4);
  }
  ...
  void _handleCommand(_Command command) {
    switch (command.code) {
      case _Codes.init:
        ...
        _sendPort.send(
          _Command(_Codes.init, args: [rootIsolateToken, _interpreter.address, _labels]),
        );
      case _Codes.ready:
        _isReady = true;
      case _Codes.busy:
...
class _DetectorServer {
  ...
  Future<void> _handleCommand(_Command command) async {
    switch (command.code) {
      case _Codes.init:
        ...
        _numClasses = outputTensors[0].shape[1] - 4;
        _numBoxes = outputTensors[0].shape[2];
        _resolution = inputTensors[0].shape[1];

        assert(inputTensors[0].shape[1] == inputTensors[0].shape[2]);
        assert(_numClasses == _labels!.length);
  ...
  Map<String, dynamic> analyseImage(image_lib.Image? image, int preConversionTime) {
    ...
    final output = _runInferenceYolo(imageMatrix);
    ...
    final List<Rect> locations =
        locationsRaw
            .map((list) => list.map((value) => (value * _resolution!)).toList())
