...
image_lib.Image convertNV21ToImage(CameraImage cameraImage) {
  ...
  // Create a new Image instance
  final image = image_lib.Image(width: cameraImage.width, height: cameraImage.height);

  // Convert NV21 to RGB
  convertNV21ToRGB(yuvBytes, vuBytes, cameraImage.width, cameraImage.height, image);
...
Future<void> saveImage(image_lib.Image image, String path) async {
  Uint8List bytes = image_lib.encodeJpg(image);
  final fileOnDevice = File(path);
