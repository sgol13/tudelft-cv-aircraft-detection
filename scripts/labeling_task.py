...
def verify_task(videos_dir: str, output_dir: str, list_tasks: bool):
    task_videos = set(file for file in os.listdir(videos_dir) if file.endswith('.mp4'))
    labeled_videos = set(f'{file.split(".")[0]}.mp4' for file in os.listdir(output_dir) if file.endswith('.xml'))
